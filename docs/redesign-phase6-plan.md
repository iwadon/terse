...
- [x] ゴールデンテスト基盤（ヘルパ + ハイブリッド更新フロー）導入、代表 1〜2 シナリオ通過
- [x] PTY テスト基盤（POSIX 限定ヘルパ）導入、代表 1 シナリオ通過
- [x] 既存 253 件が無変更で全通過
- [x] `include/terse.h` の差分が空（破壊的変更なし・公開 API 不変）
- [x] `UPDATE_GOLDEN=1` 更新 → 未設定比較のラウンドトリップが機能
- [x] ドキュメント反映（progress-overview / 本計画書の確定事項）
- [x] Conventional Commits でコミット（push は Phase 6 完了後に一括 → Phase 5.5 へ）

---

## 7. 実装結果（2026-05-29）

3 コミットで完了。全テスト 257 件通過（既存 253 + ゴールデン 2 + PTY 2）、
`include/terse.h` 不変（公開 API に一切触れていない）。

- **コミット 1（`fb7ba9a`）**: ゴールデンテスト基盤。
  - `tests/golden_helpers.h`: `golden_begin`（pipe + `terse_open`）/ `golden_read_all`
    （close → 全 read、EAGAIN 時は 1ms sleep 再読み）/ `golden_escape`（ESC→`\x1b`、
    LF→`\n`+実改行、CR/TAB、その他制御・非ASCII→`\xNN`、印字 ASCII はそのまま）/
    `golden_assert`（`UPDATE_GOLDEN=1` で上書き、それ以外は比較、未設定かつ不在は明示失敗）/
    `golden_capture_assert`（read→escape→assert→free の定型）。
  - `tests/unit/terse_golden_test.c`: 2 シナリオ。`enabled_caps` で段階固定（端末名非依存）。
    - `truecolor_foreground` → `\x1b[0m\x1b[38;2;18;52;86m\x1b[0m`
    - `basic16_foreground_effects` → `\x1b[0m\x1b[1;4;31;106m\x1b[0m`
    - 末尾 `\x1b[0m` は `terse_close` のリセット。close 後に read するため含まれ、
      「最終的に端末へ出るバイト列」として正しい検証対象。
  - `tests/golden/`: ゴールデン格納（LF 固定 `.gitattributes`）。
  - `tests/CMakeLists.txt`: `TERSE_GOLDEN_DIR` コンパイル定義（ソースツリー内）。
- **コミット 2（`b80568f`）**: PTY テスト基盤。
  - `tests/pty_helpers.h`: `pty_open_pair` / `pty_set_winsize`（`terse_get_size_test.c`
    のローカル手順を昇格）。POSIX 限定。
  - `tests/unit/terse_pty_test.c`: 2 シナリオ。`GetSizeReturnsWinsize`（TIOCGWINSZ 経路）、
    `SgrOutputReachesMaster`（slave へ書いた SGR を master で read。実 tty 経由 I/O 実証）。
    PTY 非対応環境は早期 return スキップ。
- **コミット 3**: ドキュメント反映（progress-overview / 本節）。

### 実装中に確定した設計事実

- **`terse_get_size` はキャッシュする**（`terse.c:577` `if (!handle->size.known) refresh_size`）。
  PTY で `TIOCSWINSZ` 後に再取得しても古い値を返すため、リサイズ後再取得シナリオは
  SIGWINCH/resize イベントによる `size.known` 無効化の配線確認が前提になる。今フェーズは
  最小スコープのため見送り、サイズ取得は「初回 PTY 上で成功する」までを検証対象とした。
- PTY の `SgrOutputReachesMaster` でエコー混入の懸念があったが、検証対象は slave への
  **出力**（terse が書いたバイト列）であり、エコーは「master の入力 → slave が echo」の
  逆方向のため混入しない。複数回実行で安定確認済み。

### 今後（Phase 6 のスコープ外として残すもの）

- 既存 253 件の段階的ゴールデン化（一括書き換えはしない方針）。
- 端末別スナップショットの網羅（iTerm2 OSC1337 / kitty / Sixel / Human68k 固有）。
- 仮想セルバッファのスナップショットテスト（バッファ状態の直接アサート）。
- Windows 版 PTY テスト（ConPTY）。
- リサイズ/SIGWINCH を絡めた PTY シナリオ（size 無効化経路の確認とセット）。

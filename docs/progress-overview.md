...
| 矩形バッファ一般化（任意 origin の仮想画面） | P0+ | ✅ 実装済み | バッファドモードを端末上の任意矩形 `(origin, rows, cols)` に対応する仮想画面へ一般化。バッファ内座標はローカル(0起点)、`flush` で `絶対 = origin + local` に射影。`terse_buffer_set_region()` で origin 移動・サイズ変更（内容非保持・次 flush で全再描画）。`terse_options_t.buffer_origin_*/buffer_rows/buffer_cols`（全0=端末全体・既定）で初期矩形指定。origin 移動・縮小時は前回矩形の残骸を `flush` が自動消去（terse 自身が出したセルのみ）。`terse_get_cell()` で表示中の確定内容を読み出し。リサイズ追従は利用側ポーリング（`terse_get_size()`→`terse_buffer_set_region()`）方式。|

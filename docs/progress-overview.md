...
- `-DTERSE_ENABLE_ICONV=OFF` で外部 iconv 非依存ビルドが可能に。内蔵の mini iconv が `Shift_JIS` ⇔ `UTF-8` 相互変換を提供する（背景・設計は `docs/mini-iconv-plan.md` を参照）。
...
- 画像/クリップボードの互換拡大（iTerm2以外のSixel/kitty graphics対応を含む）と能力表の詳細化。
- ドキュメントの仕様整備とサンプルの拡充（P2/P3機能別の最小コード例）。
- kitty キーボードプロトコルのハンドシェイクシーケンスを実機（Kittyアプリ）で再検証し、必要なら送出列を更新。

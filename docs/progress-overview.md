...
## Summary
P0のライフサイクル・出力・入力が一通り整い、ANSIシーケンス送出・イベント正規化・`-errno`返却・`get_size` まで実装済み。オプションでは機能無効化フラグとアクセサを備え、縮退パスも確認済み。最新の作業で Apple Terminal および VTE 系向けに環境ベースの capability 推定を追加し、P1/P2 プロファイル判定の足がかりができた。残作業は状態保持や上位プロファイル機能の実装拡張。以下の表で仕様領域ごとの状態を俯瞰できます。
...
| 環境検出・プロファイル自動判定 | P0-P2 | ⚠️ 部分 | Apple Terminal と VTE (GNOME Terminal) の識別と真偽色/マウス/P1-2 capability への自動移行を実装。その他端末は未対応。 |
...
- 追加端末（iTerm2、WezTerm、kitty、Warp 等）の DA 収集とマッピング実装。

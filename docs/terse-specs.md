...
- **codepoint**: 入力文字のコードポイント。符号化に依存するが抽象的にはUnicodeスカラー値。未知／非対応は置換文字。
...
- Codecは `open(..., options.codec)` にて指定可能。
- 実装は少なくとも `UTF-8` と `Shift_JIS` を必須サポートとする。
- その他のローカルコードページは実装依存で追加可能。

### 出力規約

- 抽象文字列は `write_text` 内で選択されたCodecに従い符号化される。
- 符号化不可能な文字は **代替文字**に置換する。
  - UTF-8: U+FFFD (�) または `?`（実装ポリシー）。
  - Shift_JIS: `?`（0x3F）。
- 改行はLF (U+000A) を基本とし、CRLFは扱わない。

### 入力規約

- 受信バイト列は選択Codecで復号する。
- 不正シーケンスは以下のいずれかの挙動を選択可能（実装オプション、規約で明記）：
  1. **代替文字置換**（UTF-8: U+FFFD、Shift_JIS: `?`）
  2. **RawSequence** として返却し、アプリ側に委ねる。
- 制御文字 (C0/C1) はキーイベントへ正規化される（Enter, Tab, Backspace等）。未知制御はRawSequence。
...
- セッション途中でCodecを変更することは想定しない（再 `open` が必要）。
- 入出力で異なるCodecを使うことは許容しない。
...
3. **合成文字**
   - UTF-8では結合文字を許容し、`width=0` を付与（9章）。
...
- 半角カナ（0xA1–0xDF）はそのままスカラーへ写像し幅1（9章）。

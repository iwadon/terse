...
/*
 * Unicode character width calculation based on East Asian Width property.
 * Supports combining characters (width=0), wide characters (width=2),
 * and ambiguous width characters (configurable).
 */
...
/* Binary search helper for Unicode intervals */
static int
...
/* Wide characters (full-width, width=2) */
...
/* Ambiguous width characters (configurable as 1 or 2) */
...
terse_compute_cell_width(terse_handle_t handle, unsigned int scalar)
{
	...
	if (terse_is_combining(scalar)) {
		return 0;
	}
	/* Wide characters have width 2 */
	if (terse_is_wide(scalar)) {
		return 2;
	}
	/* Ambiguous width characters depend on configuration */
	if (terse_is_ambiguous(scalar)) {

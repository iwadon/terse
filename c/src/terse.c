...
static const unicode_interval_t ambiguous_intervals[] = {
	{ 0x00a1, 0x00a1 },
	{ 0x00a4, 0x00a4 },
	{ 0x00a7, 0x00a8 },
	{ 0x00aa, 0x00aa },
...
static int compute_cell_width(terse_handle_t handle, unsigned int scalar);
...
static int
compute_cell_width(terse_handle_t handle, unsigned int scalar)
{
	if (scalar == 0) {
		return 0;
	}
	...
	if (interval_contains(ambiguous_intervals, sizeof(ambiguous_intervals) / sizeof(ambiguous_intervals[0]), scalar)) {
		return (handle && handle->options.east_asian_ambiguous_as_wide) ? 2 : 1;
	}
	return 1;
}
...
{
	int width = compute_cell_width(handle, scalar);
